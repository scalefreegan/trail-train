 ...
 * @param {{now?: Date|number|string, cutoffH?: number|null}} [opts] `now` is
 *   the instant a bare HH:MM is read against; defaults to the wall clock.
 *   `cutoffH` is the race's own `cutoff_h` — when given (a finite number), a
 *   bare HH:MM can never resolve to an occurrence after the course closes
 *   (plus RACE_WINDOW_SLACK_H), no matter how far past that point `now`
 *   actually is (PR #24 review round 3: RaceDay.tsx calls this for a manual
 *   hold even when the race is long past, and an unclamped `now` used to let
 *   the answer land up to MAX_SPAN_DAYS out). Without it, MAX_SPAN_DAYS is
 *   the only ceiling, same as before this option existed.
