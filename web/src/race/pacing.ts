...
/*                                                                     */
/*  The OLS fit is a faithful TS port of fitPacing() in                */
/*  scripts/facts.mjs (kept in sync by hand — change both). It runs    */
/*  client-side on useStrava().activities so the planner sliders       */
/*  recompute live without a sync round-trip.                          */
/* ------------------------------------------------------------------ */
...
export type PacingFit = {
  ...
  kDist: number;
  /** residual std dev, s/mi — the ± band */
  residStd: number;
  n: number;
};

type FitInput = { distance_mi: number; elevation_ft: number; moving_s: number };

export function fitPacing(acts: FitInput[]): PacingFit | null {
  const rows = acts
  ...
  const residStd = Math.sqrt(resid.reduce((s, e) => s + e * e, 0) / resid.length);

  return { base, kVert, kDist, residStd, n: rows.length };
}
...
export type ProjectOptions = {
  /** pace degradation per 10 miles, e.g. 4 → +4%/10mi */
  fatiguePctPer10mi: number;
  /** target finish in hours, or null to skip the goal overlay */
  goalH: number | null;
  dwellMin?: number;      // minutes at a regular aid station
  crewDwellMin?: number;  // minutes where crew/drop bag access exists
};
 ...
 *
 * Modeling note: kDist was fitted on training runs ≤ ~26 mi. Linearly
 * extrapolating it to mile 100 double-counts fatigue and explodes, so the
 * fitness pace is evaluated at a fixed reference distance (D_REF = 20 mi,
 * the athlete's long-run regime) and ALL ultra-distance slowdown comes from
 * the explicit fatigue multiplier `1 + f·(mi/10)`. Don't "fix" this back to
 * kDist·total_mi.
 */
const D_REF = 20;
...
export function projectRace(course: Course, fit: PacingFit, opts: ProjectOptions): RaceProjection {
  ...
  const dwellS = (opts.dwellMin ?? 3) * 60;
  const crewDwellS = (opts.crewDwellMin ?? 8) * 60;
  ...
  // moving time per segment per scenario (before dwell)
  ...
  const segTimes = segs.map(({ seg_mi, seg_gain_ft, prevMi }) => {
    ...
    for (const sc of ["best", "avg", "worst"] as Scenario[]) {
      ...
      out[sc] = seg_mi * pace * mult; // seconds
    }
    return out;
  });

  // dwell applies at every intermediate station once you've arrived there —
  // so a station's own arrival excludes its dwell, later stations include it.
  const dwellAt = (st: CourseAidStation, isLast: boolean) =>
    isLast || st.crew_only ? 0 : (st.crew || st.drop_bag ? crewDwellS : dwellS);
  ...
  for (const sc of ["best", "avg", "worst"] as Scenario[]) {
    ...
    segs.forEach(({ st }, i) => {
      t += segTimes[i][sc];
      arrive[sc].push(t);
      t += dwellAt(st, i === segs.length - 1);
      depart[sc].push(t);
    });
  }

  // goal overlay: scale avg *moving* time to hit goalH, dwell held constant
  const totalDwellS = segs.reduce((s, { st }, i) => s + dwellAt(st, i === segs.length - 1), 0);
  const avgMovingTotalS = segTimes.reduce((s, t) => s + t.avg, 0);
  let goalArrive: number[] | null = null;
  if (opts.goalH != null && avgMovingTotalS > 0) {
    const goalMovingS = Math.max(0, opts.goalH * 3600 - totalDwellS);
    const scale = goalMovingS / avgMovingTotalS;
    let moving = 0, dwell = 0;
    goalArrive = segs.map(({ st }, i) => {
      moving += segTimes[i].avg * scale;
      const at = moving + dwell;
      dwell += dwellAt(st, i === segs.length - 1);
      return at;
    });
  }
  ...
  return { stations: projections, finish_h, goal_h: opts.goalH, elapsedAtMile, mileAtElapsed };
}
